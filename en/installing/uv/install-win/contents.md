...
title: Python setup with uv (Windows)
tutorial_questions:
  setup:
...
> This guide assumes you can already open PowerShell and move between folders with `cd`. If that is new to you, first work through the command line tutorial.
...
Then reopen PowerShell and check that it works:
...
Ideally, your work is automatically backed up. Unfortunately, saving Python projects in OneDrive is a bit of a problem. Hence, you will install Nextcloud, as provided by the University of Amsterdam. It will take care of automatic backup.
...
> **Do not save your Python work on OneDrive.** You should have installed Nextcloud in the previous step.
...
Let's say that you are using `$HOME\Nextcloud\Programming` as your programming folder. Now it's time to create a course-specific subfolder.
...
~~~powershell
cd $HOME\Nextcloud\Programming\course-with-project
uv sync
~~~
...
- create a new course subfolder inside `$HOME\Nextcloud\Programming`
...
Not sure whether everything above actually worked? Paste the command below into PowerShell. It checks that `uv` is installed, that it can run a recent enough Python version, and that your `Nextcloud` folder is set up correctly (including the `.venv` exclusion).
