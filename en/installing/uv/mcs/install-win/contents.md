...
This file lists the packages you need for this course. Save it in your course folder, so that you have `$HOME\Programming\MCS\requirements.txt`.

Make sure you still have the uv environment activated (`(uv-sp)` in your prompt). Then install everything it lists:

~~~powershell
cd $HOME\Programming\MCS
...
3. Go to the folder you are working in, for example `cd $HOME\Programming\MCS\module1`.
